...
void MainWindow::showRoomTable(RoomList* roomList) {
    ...
    ROOMLIST::iterator room;
    int i = 0;
    for (room = roomList->roomList.begin(); room != roomList->roomList.end(); room++, i++) {
        model->setItem(i, 0, new QStandardItem(QString::number((*room)->roomNumber)));
        model->setItem(i, 1, new QStandardItem((*room)->getRoomTypeName()));
    ...
    ui->statusbar->showMessage(QString("Room list load compelete, number = %1").arg(roomList->roomList.size()));
}
...
void MainWindow::showCustomerTable(CustomerList *customerList) {
    ...
    int i = 0;
    for (customer = customerList->list.begin(); customer != customerList->list.end(); customer++, i++) {
        model->setItem(i, 0, new QStandardItem(QString::number((*customer)->customerID)));
        model->setItem(i, 1, new QStandardItem((*customer)->getCustomerTypeName()));
    ...
    ui->tableView->resizeColumnsToContents();
    ui->statusbar->showMessage(QString("Customer list load compelete, number = %1").arg(customerList->list.size()));
}
...
void MainWindow::showAllTable(RoomList *roomList) {
    ...
    for (room = roomList->roomList.begin(); room != roomList->roomList.end(); room++, i++) {
        ...
        if (customer != NULL) {
            ...
            model->setItem(i, 7, new QStandardItem(customer->getCustomerTypeName()));
...
void MainWindow::dateRangeHandler(DateRange dateRange) {
    ...
    ROOMLIST::iterator room = roomList->roomList.begin();
    while (room != roomList->roomList.end()) {
        Customer* customer = database.getCustomerDataByRoom
                ((*room)->roomNumber);
        if (customer != NULL) {
            /* if the room in this time range has person living, skip it */
